...
When [booting from the ISO]({{< baseurl >}}/os/v1.x/cn/installation/running-rancheros/workstation/boot-from-iso/), RancherOS starts with the default console, which is based on busybox.

You can select which console you want RancherOS to start with using the [cloud-config]({{< baseurl >}}/os/v1.x/cn/installation/configuration/#cloud-config).

### Enabling Consoles using Cloud-Config

When launching RancherOS with a [cloud-config]({{< baseurl >}}/os/v1.x/cn/installation/configuration/#cloud-config) file, you can select which console you want to use.
...
> **Note:** When using a persistent console and in the current version's console, [rolling back]({{< baseurl >}}/os/v1.x/cn/upgrading/#rolling-back-an-upgrade) is not supported. For example, rolling back to v0.4.5 when using a v0.5.0 persistent console is not supported.
