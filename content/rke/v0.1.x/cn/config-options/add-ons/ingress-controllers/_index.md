...
The images used for ingress controller is under the [`system_images` directive]({< baseurl >}}/rke/v0.1.x/cn/config-options/system-images/). For each Kubernetes version, there are default images associated with the ingress controller, but these can be overridden by changing the image tag in `system_images`.
